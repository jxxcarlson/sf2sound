...
% \abstract{foo, bar}{}
\begin{abstract}
We report on {\tt sf2sound}, a unix command-line program to create sound files from text such as
...
by {\tt sf2sound}.  The symbol {\tt \ } is used  as with any unix command to
...
\end{verbatim}
In its current implementation, {\tt sf2sound} can handle up to ten voices may.
...
The symbols {\tt |} and {\tt .} have no meaning for {\tt sf2sound} and are ignored
...
It is apparent from the foregoing that the input to {\tt sf2sound}, be it 
...
The program {\tt sf2sound} produces audio from text by applying a sequence of transformations, as pictured below. 
...
the sound file written by {\tt sf2sound ex1 'do re mi'}.  Then $s_i = \phi(t_i)$.
